...
> **软件官网（国内可访问）**：https://quark.bacon123.eu.org/
>
> **中转链接订阅页**：https://quark.bacon123.eu.org/go
>
> **建议**：将 `https://quark.bacon123.eu.org/go/资源名` 作为长期订阅地址，源站更新夸克原链后可自动跟随。
...
我们提供的中转链接订阅入口：**https://quark.bacon123.eu.org/go**

用法建议：

1. 在 Cloudflare Worker 部署中转页（当前项目已内置 `/go` 与 `/go/:资源名`）。
2. 对外只分发中转链接，例如 `https://quark.bacon123.eu.org/go/剑来2`（或你自己的域名）。
...
### v1.0.17 (2026-03-11)
- PotPlayer 实时进度采集改为优先按启动进程 PID 精确关联文件，修复剧集类视频进度卡在旧值的问题
- 保留标题匹配与 DPL 记录作为回退，避免手动启动播放器时完全失去进度刷新

### v1.0.16 (2026-03-11)
- 播放进度采集优化：优先实时读取 PotPlayer 当前播放位置，DPL 记录继续作为兜底
- 保留现有自动清理策略，视频达到阈值后仍按播放进度执行删除

### v1.0.14 (2026-03-08)
- 授权校验链路升级：客户端优先使用 WebSocket 获取实时授权状态，HTTP checkin 作为回退，显著降低高频 KV 请求
- Durable Object 新增授权状态缓存与在线状态聚合，后台“在线时间”改为更准确的累计值
- 在线时长持久化：总在线时长写入 DO Storage，Worker 重启后仍可保留历史累计时长
- 管理后台用户列调整为「最后上报时间 / 上报次数 / 在线时间 / 最后一次上报内容」
- 客户端购买入口链接同步更新为新的闲鱼链接
